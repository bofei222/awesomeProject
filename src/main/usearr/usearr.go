...
func generateTurbineData(startTime int64) []*WindTurbineData {
	...
	for i := int64(0); i < 3600; i++ {
		...
		for j := 0; j < 1000; j++ {
			floatData[j] = rand.Float32() * 1000.0 // 假设 float 数据范围 0-100
...
func main() {
	...
	go func() {
		...
		for range ticker.C {
			startTime := time.Now()
			avg := calculateAverage(windTurbines[3])
			cost := time.Since(startTime)
			fmt.Printf("每 30 秒查询耗时：%v风机 0003 的平均值: %+v\n", cost, avg)
		}
	}()
	...
	go func() {
		...
		for range ticker.C {
			startTime := time.Now()
			avgAll := calculateAllTurbinesAverage(windTurbines)
			cost := time.Since(startTime)
			fmt.Printf("每 3 分钟查询耗时：%v：全场风机的平均值: %+v\n", cost, avgAll)
		}
	}()
